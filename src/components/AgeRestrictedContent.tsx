...
import { Baby, Star, Heart, Sparkles, Users, TrendingUp } from 'lucide-react';
...
export const AgeRestrictedContent: React.FC<AgeRestrictedContentProps> = ({ profile }) => {
  ...
  // Popular content for different age groups
  ...
  if (profile.is_child) {
    return (
      <div className="space-y-8">
        ...
        {(!animationMovies?.results?.length && !familyMovies?.results?.length && !adventureMovies?.results?.length && !kidsTV?.results?.length) && (
