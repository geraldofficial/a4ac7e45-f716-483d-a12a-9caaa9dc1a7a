...
class EnhancedDatabaseWatchPartyService {
  ...
  async createSession(
    ...
    const { error } = await supabase.from("watch_party_sessions").insert({
      id: sessionId,
      host_id: user.id,
      movie_id: movieId,
  ...
  async joinSession(
    sessionId: string,
  ): Promise<EnhancedWatchPartySession | null> {
    const {
      data: { user },
    } = await supabase.auth.getUser();
    if (!user) throw new Error("User not authenticated");
  ...
  private async addParticipant(sessionId: string, user: any): Promise<void> {
    ...
    if (error && !error.message.includes("duplicate")) {
      ...
      console.error("Error adding participant:", errorMessage, error);
    }
  ...
  private async getParticipants(
    sessionId: string,
  ): Promise<EnhancedWatchPartyParticipant[]> {
    const { data, error } = await supabase
  ...
  subscribeToSession(
    sessionId: string,
    callback: (session: EnhancedWatchPartySession) => void,
  ): () => void {
  ...
  subscribeToMessages(
    sessionId: string,
    callback: (messages: EnhancedWatchPartyMessage[]) => void,
  ): () => void {
  ...
  subscribeToPlaybackSync(
    sessionId: string,
    callback: (data: PlaybackSyncData) => void,
  ): () => void {
...
export const enhancedDatabaseWatchPartyService =
  new EnhancedDatabaseWatchPartyService();
