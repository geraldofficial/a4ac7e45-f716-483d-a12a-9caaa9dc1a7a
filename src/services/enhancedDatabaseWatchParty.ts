...
class EnhancedDatabaseWatchPartyService {
  ...
  async joinSession(sessionId: string): Promise<EnhancedWatchPartySession | null> {
    ...
    if (sessionError || !session) {
      const errorMessage = sessionError ?
        (sessionError.message || sessionError.details || sessionError.hint || 'Unknown error') :
  ...
  private async addParticipant(sessionId: string, user: any): Promise<void> {
    ...
    if (error) {
      console.error("Error syncing playback:", error instanceof Error ? error.message : String(error));
    }
    }
