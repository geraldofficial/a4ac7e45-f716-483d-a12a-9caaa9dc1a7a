...
class WatchPartyService {
  ...
  private listeners: Map<string, (session: WatchPartySession) => void> =
    new Map();
  ...
  async joinSession(sessionId: string): Promise<WatchPartySession | null> {
    ...
    try {
      ...
      if (!existingParticipant) {
        ...
        localStorage.setItem(
          this.getChatKey(cleanSessionId),
  ...
  sessionExists(sessionId: string): boolean {
    ...
    const sessionData = localStorage.getItem(
      this.getStorageKey(cleanSessionId),
    );
  ...
  subscribeToSession(
    sessionId: string,
    callback: (session: WatchPartySession) => void,
  ): () => void {
  ...
  cleanupOldSessions(): void {
    try {
      ...
      console.log(
        `Cleaned up ${allSessions.length - validSessions.length} expired sessions`,
      );
    } catch (error) {
