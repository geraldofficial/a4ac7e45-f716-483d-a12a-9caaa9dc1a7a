...
export const useRobustAuthState = () => {
  ...
  const { createBasicUserProfile, fetchAndMergeProfile } =
    useAuthProfileManager();
      ...
      try {
        ...
        safetyTimeout = setTimeout(() => {
          if (mountedRef.current && loadingRef.current) {
            console.log(
              "⏰ Safety timeout: forcing loading to false after 10 seconds",
            );
            setLoading(false);
              ...
              if (event === "SIGNED_IN" && session?.user) {
                ...
                if (mountedRef.current) {
                  ...
                  setTimeout(async () => {
                    if (mountedRef.current) {
                      try {
                        const fullUser = await fetchAndMergeProfile(
                          session.user.id,
                          basicUser,
                        );
                        if (mountedRef.current) {
                          setUser(fullUser);
                          console.log(
                            "🎉 Full user profile loaded successfully",
                          );
                        }
                      } catch (profileError) {
                        console.warn(
                          "⚠️ Profile loading failed, keeping basic user:",
                          profileError,
                        );
                      }
                    }
                  ...
                  if (mountedRef.current) {
                    try {
                      ...
                      if (mountedRef.current) {
                        setUser(fullUser);
                        console.log(
                          "🎉 Existing user profile loaded successfully",
                        );
                      }
                    } catch (profileError) {
                      console.warn(
                        "⚠️ Existing profile loading failed, keeping basic user:",
                        profileError,
                      );
                    }
                  }
